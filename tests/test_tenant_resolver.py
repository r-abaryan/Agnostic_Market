...
def test_button_resolves_known_merchant(registry: ConfigRegistry) -> None:
    resolver = TenantResolver(registry)
    assert resolver.resolve_by_button("demo_shop") == "demo_shop"


def test_button_rejects_unknown_merchant(registry: ConfigRegistry) -> None:
    resolver = TenantResolver(registry)
    with pytest.raises(TenantResolutionError):
        resolver.resolve_by_button("ghost_store")
